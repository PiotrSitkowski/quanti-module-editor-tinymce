...
/**
 * EditorWysywigMceModule — Fleet Worker (WorkerEntrypoint)
 *
 * CRUD for editor_wysywig_mce via BACKEND.sys_executeDbQuery() proxy.
 ...
 * Queue:      dot.notation events (editor_wysywig_mce.created / .updated / .deleted)
 */
...
export default class EditorWysywigMceModule extends WorkerEntrypoint<Env> {
    ...
    async list(traceId: string, payload: any): Promise<any[]> {
        ...
        let sql = 'SELECT * FROM editor_wysywig_mce WHERE project_id = ? AND instance_key = ?';
    ...
    async getById(traceId: string, payload: any): Promise<any | null> {
        ...
        const result = await db(this.env, projectId, traceId)
            .prepare('SELECT * FROM editor_wysywig_mce WHERE id = ? AND project_id = ? AND instance_key = ?')
    ...
    async create(traceId: string, payload: any): Promise<any> {
        ...
        await db(this.env, projectId, traceId)
            .prepare(`INSERT INTO editor_wysywig_mce (id, project_id, instance_key, metadata, created_at, updated_at)
        ...
        if (this.env.QUEUE_MAIN) {
            this.ctx.waitUntil(
                this.env.QUEUE_MAIN.send({
                    ...
                    type:     'editor_wysywig_mce.created',
                    version:  1,
    ...
    async update(traceId: string, payload: any): Promise<any | null> {
        ...
        await db(this.env, projectId, traceId)
            .prepare(`UPDATE editor_wysywig_mce SET ${updates.join(', ')} WHERE project_id = ? AND instance_key = ? AND id = ?`)
            .bind(...params).run();
        ...
        if (this.env.QUEUE_MAIN) {
            this.ctx.waitUntil(
                this.env.QUEUE_MAIN.send({
                    event_id: crypto.randomUUID(),
                    type:     'editor_wysywig_mce.updated',
                    version:  1,
                    context:  { projectId, instanceKey, traceId },
                    data:     { id },
                })
    ...
    async delete(traceId: string, payload: any): Promise<boolean> {
        ...
        await db(this.env, projectId, traceId)
            .prepare('DELETE FROM editor_wysywig_mce WHERE id = ? AND project_id = ? AND instance_key = ?')
        ...
        if (this.env.QUEUE_MAIN) {
            this.ctx.waitUntil(
                this.env.QUEUE_MAIN.send({
                    event_id: crypto.randomUUID(),
                    type:     'editor_wysywig_mce.deleted',
                    version:  1,
                    context:  { projectId, instanceKey, traceId },
                    data:     { id },
                })
