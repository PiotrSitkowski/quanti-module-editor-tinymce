/**
 * Queue Consumer — EditorWysywigMceModule
 *
    ...
    async queue(batch: MessageBatch<QueueEvent>, env: Env): Promise<void> {
        for (const msg of batch.messages) {
            try {
                ...
                switch (event.type) {
                    case 'editor_wysywig_mce.created':
                        // TODO: Handle creation event
                        // e.g., generate embeddings, send notifications
                        console.log(`[${event.context.traceId}] editor-wysywig-mce created: ${event.data.id}`);
                        break;

                    case 'editor_wysywig_mce.updated':
                        // TODO: Handle update event
                        console.log(`[${event.context.traceId}] editor-wysywig-mce updated: ${event.data.id}`);
                        break;

                    case 'editor_wysywig_mce.deleted':
                        // TODO: Handle deletion event
                        console.log(`[${event.context.traceId}] editor-wysywig-mce deleted: ${event.data.id}`);
                        break;
